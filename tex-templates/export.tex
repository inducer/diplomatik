...
% per-degree data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#foreach ($deg_loop in $h.eletterate($h.sortBy($student.Degrees.values(), "EnrolledDate"), 3, $none))##
  #set ($dletter = $deg_loop.Letter)##
  #if ($deg_loop.Active)##
    #set ($degree = $deg_loop.Data)##

    \def\ABSCH${dletter}GUELTIG{1}
    \def\ABSCH${dletter}BEGINN{$h.formatDate($degree.EnrolledDate)}
    ...
    #set ($components = $h.alistKeys($drs.degreeComponents()))##
    #foreach ($component in $components)##
      #foreach ($ex_loop in $h.eletterate($h.filterBy($exams, "DegreeComponent", $component), $drs.expectedExamCount($component), $none))##
        #set ($eletter = $ex_loop.Letter)##
            ...
            \def\ABSCH${dletter}KOMP${component}PRF${eletter}URSPRUNG{$h.escapeTeX($h.alistLookup($drs.examSources(), $exam.Source))}
...
% compatibility
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\DATUM{\ABSCHLUSSbBEGINN}
\def\VORDIPL{\ABSCHLUSSaENDE}
\def\TECHFACH{\ABSCHLUSSaNF}
\def\Semesterzahl{\HOCHSCHULSEMESTER}
\def\Abschlussdatum{\ABSCHLUSSbENDE}
\def\PRUEFORDDAT{03.06.1983}% Datum der rel. Pruefungsordnung
\def\PRUEFORDPARA{\S 10 (2)}% Pruefungsparagraph
