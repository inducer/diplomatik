\input header
\def\DATUM{$h.formatDate($degree.EnrolledSemester.startDate())}
\def\NAME{$h.escapeTeX($student.LastName)}
\def\VORNAME{$h.escapeTeX($student.FirstName)}
#if ($student.DateOfBirth)
  \def\GEBDAT{$h.formatDate($student.DateOfBirth)}
#else
  \def\GEBDAT{}
#end
\def\GEBORT{$h.escapeTeX($student.PlaceOfBirth)}
\def\MATRIKEL{$student.ID}
\def\STUDBEG{$studienbeginn_sem}
\def\VORDIPL{$h.formatDate($vddat)}
\def\TECHFACH{$h.escapeTeX($degree.MinorSubject)}
\def\URLAUBSSEMSTER{$urlaubssem}
\def\HOCHSCHULSEMESTER{$semzahl}
\def\FACHSEMESTER{$fachsem}
#if ($urlaubssem)
  \def\URLAUBSSEMESTERTEXT{ (+$urlaubssem Urlaubssem.)}
#else
  \def\URLAUBSSEMESTERTEXT{}
#end
\def\PRUEFUNGSORDNUNG{$h.escapeTeX($drs.description())}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Abschluss
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\Semesterzahl{\HOCHSCHULSEMESTER}

$zeugnis_defs

\def\Abschlussdatum{$h.formatDate($degree.FinishedDate)}
\begin{document}
\input $form
\end{document}
